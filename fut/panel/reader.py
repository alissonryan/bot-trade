...
# The first page is bounded to 20,000 ids; its wall-clock reach depends on Jev cadence and whether
# jev_ab doubles the row rate (about 45 h at a 10 s cadence with A/B on), not on a current fixed rate.
        ...
        sql = """
            WITH facts AS (
                SELECT id, ts_ms, kind,
                       ...
                       CASE WHEN json_valid(payload) THEN substr(CAST(json_extract(payload, '$.error') AS TEXT), 1, 200) END AS error
                FROM fut_decisions
                WHERE id > ?
                ORDER BY id
                LIMIT ?
            ...
            SELECT f.id, f.ts_ms, f.kind, f.cost_usd, f.bid, f.ask, f.last, f.stale, f.spread_bps, f.error,
                   m.max_id, 0 AS sentinel
            FROM facts AS f CROSS JOIN max_row AS m
            UNION ALL
            SELECT NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, m.max_id, 1 AS sentinel
            FROM max_row AS m
            WHERE NOT EXISTS (SELECT 1 FROM facts)
        """
        rows = self._query(sql, (after_id, limit))
        max_id = int(rows[0][10] or 0) if rows else 0
        facts = [
            {"id": int(row[0]), "ts_ms": int(row[1]), "kind": row[2], "cost_usd": row[3],
             "bid": row[4], "ask": row[5], "last": row[6],
             "stale": (bool(row[7]) if row[7] is not None else None), "spread_bps": row[8], "error": row[9]}
            for row in rows if not row[11]
        ]
