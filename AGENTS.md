...
- **The counting window is bounded above too, but with a tolerance (F1).** `Store.count_writes(until_ms=...)` and `WriteMeter.counts`/`check_storm` cap the window at `now_ms + MAX_CLOCK_SKEW_MS` (`bot/ratelimit.py`, one hour), not a bare `now_ms`. Without any upper bound, a row stamped while the system clock was ahead (VM resume, a container with no RTC, an NTP step) counted as "in the last hour" until real time caught up to it — reproduced as 95 rows stamped `now + 30 days` making `check_storm` raise exit 8 at the first barrier of every boot for thirty days, with no documented escape but `KILL_WRITES_PER_HOUR=0` or deleting rows by hand. The bound must stay *tolerant*, not strict, because the pre-existing safety property for the OPPOSITE clock direction depends on it: before F1, a **backward** clock jump could only ever widen the window (no upper bound existed), so the write count could rise but never silently drop — refusing a BUY it should refuse (see `test_a_spent_budget_refuses_buy_and_still_allows_sell` in `test_collar.py` and the two clock-direction tests in `test_ratelimit.py`). A strict `until_ms=now_ms` cutoff would have broken that: a modest backward step would exclude rows written moments before it, undercounting and letting a BUY through it shouldn't. An earlier revision also had `record()`'s own prune (`Store.prune_writes(after_ms=...)`) actively DELETE rows implausibly far in the future using the same tolerance, on top of the read-side exclusion above. That delete is removed (N1): the clock direction that actually threatens this table is backward, not forward — a host with no RTC (the very environment the skew tolerance's own justification cites) boots with its clock BEHIND real time until NTP syncs, and the boot `reconcile()` in `bot/cli.py` is exactly an ordinary write that can run this prune with `now_ms` still lagging, so `before_ms`/`after_ms` would straddle every genuinely recent row and delete the whole ledger on the very restart this table exists to survive — reproduced as 40 rows written seconds apart surviving only until one ordinary write lands with the clock 2h behind, after which `writes_1h` reads 0 at true `now`. `prune_writes` is age-only now (`ts_ms < before_ms`), so a far-future row is never counted as recent (the read-side bound above still guarantees that) but is no longer deleted for being far-future — it sits in the table, visible for forensics, and ages out through the same `before_ms` bound once real time reaches it. The accepted residual: a row stamped far ahead is invisible to counting while it is far ahead, and becomes countable only in a bounded, self-clearing window when real time catches up — an operator can recognise that as a temporary false halt from the row timestamps, not a permanent wedge. Symmetrically, a backward step larger than `MAX_CLOCK_SKEW_MS` still undercounts on the read side (a write made moments ago can fall outside `until_ms`) — that is real and untouched by N1, but it no longer destroys anything either; see the boundary tests in `test_ratelimit.py`. If bounded table growth is ever wanted, the right mechanism is a cap by row count on the monotonic `id`, which no clock can fool — not a future-prune.
