...
- `MAX_WRITES_PER_HOUR=0`, `MAX_ENTRIES_PER_DAY=0`, `KILL_WRITES_PER_HOUR=0` disable each knob independently; all three at `0` reproduce pre-P5 behaviour exactly (see `data/backtest/p5-optout-equivalence.json`). Unlike P1/P2/P4 this ships **on**, at 30 / 20 / 90: a guard-rail only counts if it is mounted before the accident. A nonzero `KILL_WRITES_PER_HOUR` below `MAX_WRITES_PER_HOUR` raises at `Settings` construction — a kill ceiling the soft gate can never reach would halt the process before the soft gate got a chance to refuse anything.
...
- The hard ceiling (`WriteMeter.check_storm`) raises `WriteStormHalt` at the `cycle.run_once` barrier — the first statement in the barrier's `try`, before any write and before the LLM section of that cycle — and `bot/cli.py` returns **exit 8**. Never mid-write. The halt message carries `LiveHands.last_stop_observation` verbatim (`stop_present` / `stop_absent` / `unknown`) and never infers one; there is no automatic resume, only a human restart after checking `order_writes` and the log.
- Paper pays the same toll as live: `PaperHands` records the same four writes a live round trip costs (entry market, resident stop, cancel, exit market) so the P0 replay is faithful and `rate_limit` can actually be exercised in paper, not only asserted never to fire.
