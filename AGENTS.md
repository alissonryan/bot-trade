...
- Age comes from the last recorded `fills.side='SELL'` timestamp in the existing bot ledger, across UTC days and process restarts, not account-wide trades or in-memory state. All confirmed exit paths already write SELL fills, including stop, local TP/TTL, flatten and reconcile. Ambiguous pending exits do not fabricate a fill; delayed reconciliation starts the clock at observation. Legacy rows without a provable SELL timestamp cannot establish an exit age.
