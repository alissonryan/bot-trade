...
5. `reconcile()` runs at boot and every LLM cycle. It never trusts the local row alone:
   ...
   - **Honest scoping (2026-09 M1 design review):** "missing ≈ our own size" is *corroboration*, not *identification*. An owner deposit/withdrawal that happens to equal the bot's own quantity produces the exact same observable delta as our own exit, and a lagging balance read can do the same — no threshold and no extra read recovers the missing causal information. Closing this gap needs the same captured order-history/deals terminal evidence the M1 exit latch (`bot/hands.py::EXIT_LATCH_KEY`, `TerminalEvidenceUnavailable`) is waiting on. It is kept anyway, deliberately: it is still the only autonomous-stop-fill detector, and dropping it would leave a real exchange-side fill undetected, which is worse. This is a documented residual risk, not a live bug being exploited today, and this note does not fix it.
6. Only bot-created order ids are ever cancelled.

Exit codes: `1` session dead, `2` unprotected position, `3` already running, `4` `--once` cycle failed, `5` stuck position (protected, but the bot cannot exit it — the resident stop is not a bot order; square it by hand), `6` terminal evidence unavailable (`TerminalEvidenceUnavailable`: a live discretionary exit — LLM SELL, local take-profit, time limit — refused before any write, because a cancel of the resident stop cannot yet be proven safe; the stop is untouched and still protects the position; capture the missing order-history/deals evidence, or exit by hand). `5` and `6` are deliberately distinct: `5` means a *foreign* stop can never be cancelled by this bot; `6` means the bot cannot yet prove a cancel of *any* stop succeeded rather than executed a moment before/after the read — the operator's remedy differs, so one exit code maps to one action instead of two.
