...
| `bot/eye.py` | Socket first (via `Hub`), REST fallback that never raises; klines (forming bar dropped), balances (**live only** — paper never calls it), symbol rules |
...
| `bot/collar.py` | Risk gate: size, ATR stop, one position, day-loss (realized + unrealized), min confidence, venue minimum. BUY sizing/stop/TP all key off `executable_entry_price()` (best ask, or `last` when the ask is missing/invalid, marked up by slippage) — the same price PaperHands actually fills at, so a declared notional cap cannot be silently overspent by pricing the size and the fill off two different numbers |
...
| `data/` | `bot.db`, `bot.log`, `bot.lock` (gitignored); path fixed to this checkout (`Path(__file__)`-relative), never the process cwd |
...
- Every audit row carries the snapshot (last/bid/ask/atr), the LLM reason/cost, and — from the boundary that actually dispatched the request — the exact sanitized request body (model/options/full messages, all candles) the decision used. Without that the paper run proves nothing and cannot be reconstructed later.
...
- Paper: KCEX login not needed. Paper's `KcexClient` always carries an explicit empty token (never falls back to reading `KCEX_TOKEN` from the environment) and `Eye.poll_heavy()` never calls the private balances endpoint in paper mode at all — cash is `PAPER_STARTING_USDT`/the paper ledger, not an exchange read.
...
- `Eye.poll_quotes()` calls `sync_hub()` first and only falls back to REST ticker+depth when WS is down or **stale** (no fresh frame within `STALE_MS`, default 30000ms — this covers both a hard socket error and a WS thread that silently stops producing frames). `poll_heavy()` (kline always; balances **live only**) is always REST.
...
- `poll_heavy()` (klines + balances) runs when the LLM is due and is always REST. The forming 15-minute bar is dropped before ATR. **Paper never calls the private balances endpoint at all** — `free_usdt` is set from `PAPER_STARTING_USDT` and the call returns before ever touching `client.balances()`, so a stale `KCEX_TOKEN` sitting in the environment cannot leak into a paper run. In live mode a balance read failure raises `EyeError` (the loop backs off) instead of trading on `free_usdt = 0`.
...
- Reflect only with at least two `LLM_FALLBACK_COST_USD` estimates remaining (this call + next decision). This is a reservation estimate, not a hard provider billing guarantee or a promise of funding all future decisions; the existing decision budget guard still applies. The daily Budget is durable per mode/database: `Store.reserve_budget()` atomically checks remaining room and commits the reservation BEFORE the HTTP dispatch (not after the cycle), so a crash mid-request still leaves it recorded; `Store.settle_budget()` trues it up to the real provider cost once known. A same-day restart resumes spend instead of silently re-authorizing the full cap; a stale persisted day rolls to zero, but a persisted day AFTER today (backward clock) refuses instead of minting a fresh budget. A corrupt/unreadable row (`BudgetStateCorrupt`) blocks new paid calls rather than resetting to zero or continuing silently — this never disables barrier/stop monitoring, only new LLM spend. A timeout/network failure reserves the conservative `LLM_FALLBACK_COST_USD` charge too (`cost_source="fallback_uncertain"`), since the provider may have billed a request this process never saw the response for. None of this is per-account or provider-wide — it is a local ledger per mode's own database, and no local cap guarantees the provider's actual bill.
...
- `FUT_MIN_HOLD_SECONDS` (default 0, must be < `FUT_MAX_HOLD_SECONDS`) suppresses Jev exit/reversal wakes for that long after entry, so the LLM is not asked to CLOSE a position seconds old. Stop, liquidation and max hold still run every step. The `jev_only` shadow computes its wake from its own position, opened timestamp, entry streak, and the same cost gate; it can enter while main is open, exits on its own reversal direction (or a carried `exit_now`), and does not inherit main's position or streak.
...
- `FUT_JEV_AB` is opt-in and off by default. When enabled, the paper loop makes one additional Jev call with semantic bucket labels immediately after the normal call; it logs `kind='jev_ab'`, doubles Jev cost (about US$0.36/day extra at 10 s), and never drives wakes, gates, orders, the ledger, or shadow books. `python -m fut jevscore [--since-ms N]` is read-only and scores both variants against a later mid; an A/B window is a new configuration and cannot count toward the edge criterion.
...
- Known limitations: model cost is incurred per elapsed paper time and the report splits it evenly across main trades for the per-trade CI, so that split is not a per-trade cost model. Jev and REST refreshes are synchronous with the loop and can leave stops, liquidation, and time-limit checks blind for up to about 2 seconds while a step is running. A funding refresh can advance `nextSettleTime` before the ledger observes the just-crossed settlement, so a narrow capture edge can miss one funding charge. The collar order is market/contract/position/day-loss, then `cost_gate`, then `entry_rate`, then confidence and the remaining sizing checks. An LLM failure/non-decision arms the same cooldown as HOLD; this is intentional and is not limited to clean HOLD JSON.
- Tests: `tests/fut/test_fut_*.py`, `tests/kcex/test_fws.py`, `tests/kcex/test_fapi.py`. No network.
- `python -m fut panel [--port 8766]` is the read-only local panel (`fut/panel/`, page `panel/index.html`). Loopback only, GET only, one refresher shared by all browser tabs, bounded incremental reads by decision id through per-read `mode=ro` connections (never `FutStore`), no network. Short reads can still contend briefly with the writer and are retried on the next refresh; during cold start the totals are incomplete and the page says so with `carregando`. Liveness is self-calibrated from the median gap between consecutive `kind='jev'` rows (`max(10 s, 3 × cadence)` capped at 120 s), with `FUT_JEV_EVERY_SECONDS` only as a fallback when fewer than five gaps have been seen. The state also exposes Jev health, and the page groups consecutive failures so the feed is not flooded. All layman wording lives in `fut/panel/narrate.py`; a new decision `kind`/`outcome`/gate rule must be narrated there or listed in `SILENT_KINDS`, otherwise it falls back to `Evento <kind>`. It shows the whole database, so it mixes sessions with different settings exactly like `fut report`.
...
- The instance lock stays shared on purpose: paper and live must never run at once. `bot/cli.py::main` acquires it (`data/bot.lock`, `fcntl.flock`) BEFORE constructing `KcexClient`/`Store`/`Eye` (WS thread, symbol-rules REST call), the optional `--chart` server and `Hands` — every one of those has a side effect, so a second racing process can no longer create/migrate the database, open a second WS connection or bind the chart port before discovering it should never have started. File logging (`add_file_logging`) is added only once the lock is held too — console logging alone covers pre-lock failures. `DATA_DIR`/`DB_PATH`/`LOCK_PATH`/`LOG_PATH` are resolved from this checkout's own location, not the process cwd, so invoking the same installed module from a different working directory can never silently pick a different ledger/lock; a genuinely separate worktree still gets its own `data/` for free. A platform with no `fcntl` fails closed (`RuntimeError`) rather than proceeding as if it held an unenforceable lock.
