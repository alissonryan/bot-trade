...
- Reflect only with at least two `LLM_FALLBACK_COST_USD` estimates remaining (this call + next decision). This is a reservation estimate, not a hard provider billing guarantee or a promise of funding all future decisions; the existing decision budget guard still applies. The daily Budget is persisted per mode/database in `Store`'s kv table (`Store.budget_load`/`budget_save`, keyed on the UTC day) and reseeded from it on process start when the persisted day still matches today — a same-day restart resumes spend instead of silently re-authorizing the full cap; a stale persisted day still rolls over to zero. A timeout/network failure now reserves the conservative `LLM_FALLBACK_COST_USD` charge too (`cost_source="fallback_uncertain"`), since the provider may have billed a request this process never saw the response for. None of this is a guarantee of the provider's actual bill, only an honest local ledger. Reflections are best-effort one-shot per resolved record, including named budget/config/offline skips; outcome-only lessons remain usable. Reflection/audit failure cannot replace a fatal hands exception, and journal settlement uses a savepoint so a metadata failure does not lose a real fill.
