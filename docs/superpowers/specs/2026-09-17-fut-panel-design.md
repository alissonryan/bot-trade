...
| `jev` com `error` | Antes de classificar, remove o identificador de requisição, URLs e tokens que contenham `=` ou `://`. Depois aplica, nesta ordem: timeout (`timed out`/`Timeout`) → "Jev demorou demais para responder"; `Connection`/`connect` → "Jev sem conexão"; status `: 429\b` ou `rate limit` → "Jev recusou por limite de uso"; status `: 401\b`/`: 403\b` → "Jev recusou a chave de acesso"; status `: 529\b`, `overloaded` ou `high traffic` → "Jev sobrecarregado (servidor da TypeSafe com excesso de demanda)"; status `: 503\b` ou `unavailable` → "Jev fora do ar (servidor da TypeSafe indisponível)"; os demais 5xx somente quando aparecem como `: 5\d{2}\b` → "Jev com erro no servidor da TypeSafe". O fallback é uma allowlist de letras, dígitos, espaços e pontuação básica, com no máximo 80 caracteres; `500 Internal Server Error` sem o prefixo SDK `: 500` cai nele. O evento recebe `grupo=jev_erro:<texto>` para o feed agrupar repetições sem expor identificadores técnicos. |
